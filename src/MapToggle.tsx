...
const MapToggle: React.FC<MapToggleProps> = ({ checked, onChange, onShowAfrica, onShowIndia, onShowEurope }) => {
  ...
  // 🔹 Active le toggle automatiquement 2 secondes après le chargement
  useEffect(() => {
    if (userInteracted) return; // ne rien faire si l'utilisateur a déjà interagi
    ...
    <>
      ...
      <Box sx={{ display: "flex", flexDirection: "column", fontFamily: "Arial, sans-serif", width: "fit-content" }}>
        <Box display="flex" alignItems="center" gap={1}>
          ...
          <Switch
            ...
            onChange={(e) => {
              ...
              setUserInteracted(true); // marque que l'utilisateur a interagi
            }}
            size="small"
            sx={{
        ...
        <Slide direction="up" in={!minimized} mountOnEnter unmountOnExit>
          <Box sx={{ mt: 0.5, maxWidth: 280 }}>
            <Typography variant="body2" sx={{ fontSize: "12px", lineHeight: 1.3, color: "#444", mb: 1 }}>
      ...
      <Slide direction="up" in={showAfricaBox} mountOnEnter unmountOnExit>
        ...
        >
          ...
          En septembre 2025, seuls 12% des pays mentionnés dans les médias français
          concernent les 54 pays du continent africain.
      ...
      <Slide direction="down" in={showIndiaBox} mountOnEnter unmountOnExit>
        ...
        >
          ...
          1,3% des pays mentionnés concernent l'Inde où vit 18% de la population mondiale.
          <Box sx={{ mt: 1, display: "flex", gap: 1 }}>
            <Button variant="outlined" size="small" onClick={handlePrevFromIndia} sx={{ borderRadius: "16px", backgroundColor: "transparent", borderColor: "#888", color: "#444", "&:hover": { backgroundColor: "rgba(0,0,0,0.05)", borderColor: "#555" }}}>Précédent</Button>
            <Button variant="outlined" size="small" onClick={handleNextAfterIndia} sx={{ borderRadius: "16px", backgroundColor: "transparent", borderColor: "#888", color: "#444", "&:hover": { backgroundColor: "rgba(0,0,0,0.05)", borderColor: "#555" }}}>Suivant</Button>
          </Box>
        </Box>
      </Slide>

      {/* Box Europe */}
      <Slide direction="down" in={showEuropeBox} mountOnEnter unmountOnExit>
        <Box
          sx={{
            position: "fixed",
            top: 20,
        ...
        >
          <Typography variant="subtitle2" sx={{ fontWeight: "bold", mb: 1, color: "#81a685ff" }}>
            Europe
          </Typography>
          XXXX% des pays mentions concernent les pays européens.
          <Box sx={{ mt: 1 }}>
            <Button variant="outlined" size="small" onClick={handlePrevFromEurope} sx={{ borderRadius: "16px", backgroundColor: "transparent", borderColor: "#888", color: "#444", "&:hover": { backgroundColor: "rgba(0,0,0,0.05)", borderColor: "#555" }}}>Précédent</Button>
          </Box>
        </Box>
      </Slide>
    </>
